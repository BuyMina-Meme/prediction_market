...
async function getDootCommitment() {
    try {
        ...
        console.error('   ⚠️  Failed to fetch Doot commitment:', e?.message || e);
        return null;
    }
}
async function trySettleMarket(marketId, marketAddress58) {
...
export async function startSettlementMonitor() {
    ...
    const intervalId = setInterval(async () => {
        try {
            ...
            // Log and continue
            console.error('❌ Settlement monitor error:', e?.message || e);
        }
    }, config.settlement.checkInterval);
    // Return cleanup function
    return () => clearInterval(intervalId);
}
//# sourceMappingURL=settlement-monitor.js.map