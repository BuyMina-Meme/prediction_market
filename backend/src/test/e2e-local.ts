...
import { config, getDeployerKeypair } from '../config.js';
...
// @ts-ignore
import { PredictionMarket, predictionMarketOffchainState, MarketRegistry, marketRegistryOffchainState, ASSET_INDEX, MULTIPLICATION_FACTOR } from '../../contracts/build/src/index.js';
// eslint-disable-next-line @typescript-eslint/ban-ts-comment
// @ts-ignore
...
async function main() {
  // Use LocalBlockchain
  (process as any).env.LOCAL_MODE = 'true';
  (process as any).env.REDIS_HOST = 'memory';
  ...
  // Wire backend config
  (process as any).env.DEPLOYER_PRIVATE_KEY = deployer.key.toBase58();
  (process as any).env.REGISTRY_ADDRESS = registryAddress.toBase58();
  (process as any).env.DOOT_ORACLE_ADDRESS = dootAddress.toBase58();
  // Refresh config module values (already read at import time)
  (config as any).localMode = true;
  ...
  const createRes = await deployMarket(request as any);
  if (!createRes.success) throw new Error('deployMarket failed: ' + createRes.error);
  ...
  tx = await Mina.transaction(creator, async () => {
    await market.initialize(
      Field(0), // assetIndex: MINA
      Field(BigInt(createRes.initParams!.priceThreshold)),
      UInt64.from(createRes.initParams!.endTimestamp),
      creator,
      PublicKey.fromBase58(createRes.initParams!.burnAddress),
      PublicKey.fromBase58(createRes.initParams!.registryAddress)
    );
  });
  await tx.prove();
  ...
  if (!failed) throw new Error('YES bettor was able to claim unexpectedly');
