...
describe('MarketRegistry', () => {
    ...
    describe('Deployment & Initialization', () => {
        ...
        it('should initialize registry', async () => {
            ...
            assert.strictEqual(ownerAddress?.toBase58(), owner.toBase58(), 'Owner should be set');
            const marketCount = await registry.marketCount.fetch();
            assert.strictEqual(marketCount?.toString(), Field(0).toString(), 'Market count should start at 0');
        ...
        it('should prevent double initialization', async () => {
            try {
                ...
                assert.ok(error.message.includes('empty') || error.message.includes('assertEquals'), 'Should prevent double initialization');
    ...
    describe('Market Registration', () => {
        it('should register first market', async () => {
            ...
            const tx = await Mina.transaction(owner, async () => {
                await registry.registerMarket(market1Address, owner, // creator
                ASSET_INDEX.ETHEREUM, endTimestamp);
            });
            await tx.prove();
            await tx.sign([owner.key]).send();
            // Verify market count increased
            const marketCount = await registry.marketCount.fetch();
            assert.strictEqual(marketCount?.toString(), Field(1).toString(), 'Market count should be 1');
        ...
        it('should register second market', async () => {
            ...
            const tx = await Mina.transaction(owner, async () => {
                await registry.registerMarket(market2Address, owner, // creator
                ASSET_INDEX.BITCOIN, endTimestamp);
            });
            await tx.prove();
            await tx.sign([owner.key]).send();
            // Verify market count increased
            const marketCount = await registry.marketCount.fetch();
            assert.strictEqual(marketCount?.toString(), Field(2).toString(), 'Market count should be 2');
        ...
        it('should prevent non-owner from registering markets', async () => {
            ...
            try {
                const tx = await Mina.transaction(user1, async () => {
                    await registry.registerMarket(PrivateKey.random().toPublicKey(), user1, ASSET_INDEX.SOLANA, endTimestamp);
                });
                await tx.prove();
                await tx.sign([user1.key]).send();
                ...
                assert.ok(error.message.includes('assertEquals') || error.message.includes('signature'), 'Should prevent non-owner registration');
    ...
    describe('Market Retrieval', () => {
        it('should retrieve first market info', async () => {
            const tx = await Mina.transaction(deployer, async () => {
                ...
                assert.strictEqual(marketInfo.marketAddress.toBase58(), market1Address.toBase58(), 'Market address should match');
            });
            await tx.prove();
        ...
        it('should retrieve second market info', async () => {
            const tx = await Mina.transaction(deployer, async () => {
                ...
                // Verify market address matches
                assert.strictEqual(marketInfo.marketAddress.toBase58(), market2Address.toBase58(), 'Market address should match');
            });
            await tx.prove();
        ...
        it('should fail to retrieve non-existent market', async () => {
            try {
                ...
                assert.ok(error.message.includes('exist') || error.message.includes('assertLessThan'), 'Should prevent retrieval of non-existent market');
    ...
    describe('Market Status Updates', () => {
        ...
        it('should prevent non-owner from updating market status', async () => {
            try {
                ...
                assert.ok(error.message.includes('assertEquals') || error.message.includes('signature'), 'Should prevent non-owner status update');
    ...
    describe('Total Markets Query', () => {
        it('should return correct total market count', async () => {
            const tx = await Mina.transaction(deployer, async () => {
                ...
                assert.strictEqual(count.toString(), Field(2).toString(), 'Total markets should be 2');
            });
            await tx.prove();
    ...
    describe('Owner Query', () => {
        it('should return correct owner', async () => {
            const tx = await Mina.transaction(deployer, async () => {
                ...
                assert.strictEqual(ownerAddress.toBase58(), owner.toBase58(), 'Owner should match');
            });
            await tx.prove();
...
//# sourceMappingURL=MarketRegistry.test.js.map