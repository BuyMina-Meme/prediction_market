 ...
 */
import { Mina, PrivateKey, PublicKey, AccountUpdate, Field, UInt64, fetchAccount, } from 'o1js';
 ...
 */
export async function deployMarket(config, deployerKey, registryAddress, networkUrl = 'https://devnet.zeko.io/graphql') {
    try {
        console.log(`\n🏗️  Deploying market for asset index ${config.assetIndex}...`);
        // Setup network
        ...
        const deployTx = await Mina.transaction({ sender: deployer, fee: 0.1 * 1e9 }, async () => {
        ...
        const initTx = await Mina.transaction({ sender: deployer, fee: 0.1 * 1e9 }, async () => {
            // Use deployer as burn and registry address (can be updated later)
            await market.initialize(Field(config.assetIndex), Field(config.priceThreshold), UInt64.from(config.endTimestamp), creatorPublicKey, deployer, // burnAddress
        ...
        const registerTx = await Mina.transaction({ sender: deployer, fee: 0.1 * 1e9 }, async () => {
            await registry.registerMarket(marketAddress, creatorPublicKey, Field(config.assetIndex), Field(config.endTimestamp));
        });
 ...
 */
export async function deployMarkets(configs, deployerKey, registryAddress, networkUrl) {
    const results = [];
...
//# sourceMappingURL=deploy-market-programmatic.js.map